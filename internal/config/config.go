...
// Use "dkmcp serve --workspace DIR" to derive {DIR}/.sandbox/config/dkmcp.yaml,
// or "dkmcp serve --config PATH" to specify the config file directly.
// Run "dkmcp init --workspace DIR" to generate a config from the built-in template.
//
// 設定は明示的に指定されたパスからのみ読み込まれます。
// {DIR}/.sandbox/config/dkmcp.yaml を導出するには "dkmcp serve --workspace DIR" を使用し、
// 設定ファイルを直接指定するには "dkmcp serve --config PATH" を使用してください。
// テンプレートから設定を生成するには "dkmcp init --workspace DIR" を実行してください。
package config
