...
var (
	...
	// Can be set via --timeout flag or DOCKMCP_TIMEOUT environment variable.
	// The flag takes precedence over the environment variable.
	//
	// clientTimeoutはHTTPリクエストとツール呼び出しレスポンスのタイムアウト（秒）です。
	// デフォルトは30秒です。
	// --timeoutフラグまたはDOCKMCP_TIMEOUT環境変数で設定できます。
	// フラグが環境変数より優先されます。
	clientTimeout int
...
where the Docker socket is not available.`,
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			// Fall back to DOCKMCP_TIMEOUT if --timeout was not explicitly set.
			// --timeoutが明示的に指定されていない場合、DOCKMCP_TIMEOUTにフォールバックします。
			if !cmd.Flags().Changed("timeout") {
				if envTimeout := os.Getenv("DOCKMCP_TIMEOUT"); envTimeout != "" {
...
func init() {
	...
	clientCmd.PersistentFlags().IntVar(&clientTimeout, "timeout", 30,
		"Timeout in seconds for HTTP requests and tool call responses (default: 30)\n"+
			"Can also be set via DOCKMCP_TIMEOUT environment variable")
}
