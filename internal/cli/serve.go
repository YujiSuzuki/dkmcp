...
import (
	...
	"github.com/YujiSuzuki/dkmcp/internal/docker"
	"github.com/YujiSuzuki/dkmcp/internal/hosttools"
	"github.com/YujiSuzuki/dkmcp/internal/mcp"
	"github.com/YujiSuzuki/dkmcp/internal/security"
)
...
//
//	{absWorkspace}/.sandbox/config/dkmcp.yaml
//
...
func resolveConfigFile(cfgFile, flagWorkspace string) (string, error) {
	if cfgFile == "" && flagWorkspace == "" {
		return "", fmt.Errorf(
			"either --config or --workspace is required\n\n" +
				"  dkmcp serve --workspace /path/to/project\n" +
				"      Uses {workspace}/.sandbox/config/dkmcp.yaml\n\n" +
				"  dkmcp serve --config /path/to/dkmcp.yaml\n" +
	...
	resolved := filepath.Join(absWorkspace, ".sandbox", "config", "dkmcp.yaml")
	if _, err := os.Stat(resolved); errors.Is(err, os.ErrNotExist) {
		return "", fmt.Errorf(
			".sandbox/config/dkmcp.yaml not found in %s\n\nRun: dkmcp init --workspace %s",
