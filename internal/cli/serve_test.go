...
func TestResolveConfigFile(t *testing.T) {
	...
	t.Run("--config given returns it as-is", func(t *testing.T) {
		got, err := resolveConfigFile("/some/custom/dkmcp.yaml", "")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if got != "/some/custom/dkmcp.yaml" {
			t.Errorf("expected /some/custom/dkmcp.yaml, got %s", got)
		}
	})
	...
	t.Run("--workspace with missing config returns error mentioning dkmcp init", func(t *testing.T) {
		emptyDir := t.TempDir()
		_, err := resolveConfigFile("", emptyDir)
		if err == nil {
			t.Fatal("expected error when config file not found")
		}
		if !strings.Contains(err.Error(), "dkmcp init") {
			t.Errorf("expected error to mention 'dkmcp init', got: %v", err)
		}
	})
}
